...
def run_captioning(images, concept_sentence, *captions):
    """Generate captions for uploaded images using BLIP2 as a more reliable alternative"""
    device = "cuda" if torch.cuda.is_available() else "cpu"
    
    # Initialize captions list with the correct length
    captions = list(captions)
    
    try:
        # Use BLIP2 instead of Florence2 for better compatibility
        from transformers import Blip2Processor, Blip2ForConditionalGeneration
        
        processor = Blip2Processor.from_pretrained("Salesforce/blip2-opt-2.7b")
        # Load model in float32 to avoid dtype issues
        ...
        for i, image_path in enumerate(images):
            try:
                ...
                # Generate caption with BLIP2 - use consistent dtypes
                ...
                print(f"Error captioning image {i}: {str(e)}")
                captions[i] = f"a photo [trigger]" if concept_sentence else "a photo"
            
            # Yield after each image for progressive updates
            yield captions
            
    except Exception as e:
        print(f"Error loading BLIP2 model: {str(e)}")
        # Fallback to simple captions if model loading fails
...
def start_training(
    ...
    # Configure custom text encoders for T5-XXL and CLIP
    # Using T5-XXL (google/t5-v1_1-xxl) for enhanced text understanding
    # Using CLIP-ViT-L-14-336px (openai/clip-vit-large-patch14-336) for better image-text alignment
    config["config"]["process"][0]["model"]["text_encoder_2_path"] = "google/t5-v1_1-xxl"
    config["config"]["process"][0]["model"]["text_encoder_path"] = "openai/clip-vit-large-patch14-336"
    if(push_to_hub):
        try:
