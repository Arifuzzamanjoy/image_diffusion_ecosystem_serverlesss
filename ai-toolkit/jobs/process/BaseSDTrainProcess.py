...
class BaseSDTrainProcess(BaseTrainProcess):
    ...
    def sample(self, step=None, is_first=False):
        ...
        for i in range(len(sample_config.prompts)):
            ...
            # Get sample_item first (needed for seed handling)
            sample_item = sample_config.samples[i]
            
            # Handle both string prompts and dict prompts {"prompt": "...", "seed": 42}
            prompt_item = sample_config.prompts[i]
            if isinstance(prompt_item, dict):
                prompt = prompt_item.get("prompt", "")
                # Use seed from prompt dict if available (only if sample_item doesn't have one)
                if "seed" in prompt_item and sample_item.seed is None:
                    current_seed = prompt_item["seed"]
            else:
            ...
            # Override with sample_item seed if it has one
