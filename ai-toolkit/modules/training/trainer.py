...
class FluxLoRATrainer:
    ...
    def start_training(
        ...
        **kwargs
        ...
        try:
            ...
            # ADDITIONAL: Print what trainer will actually load
            self._print_trainer_data_preview(dataset_folder, concept_sentence)
    ...
    def _print_trainer_data_preview(self, dataset_folder: str, trigger_word: str) -> None:
        """Print preview of what the trainer will actually load"""
        import glob
        
        print(f"\n{'='*80}")
        print(f"🎯 TRAINER DATA PREVIEW - WHAT WILL BE LOADED:")
        print(f"{'='*80}")
        
        # Check for .txt files
        txt_files = sorted(glob.glob(os.path.join(dataset_folder, "*.txt")))[:3]
        img_files = sorted(glob.glob(os.path.join(dataset_folder, "*.jpg")) + 
                          glob.glob(os.path.join(dataset_folder, "*.png")))[:3]
        
        print(f"\n📂 Dataset Folder: {dataset_folder}")
        print(f"🔑 Trigger Word: '{trigger_word}'")
        print(f"\n📋 Config Expects: caption_ext='.txt'")
        print(f"✅ Found {len(glob.glob(os.path.join(dataset_folder, '*.txt')))} .txt files")
        print(f"✅ Found {len(img_files)} image files")
        
        print(f"\n🎯 FIRST 3 IMAGE-CAPTION PAIRS TRAINER WILL LOAD:")
        print(f"{'-'*80}")
        
        for i, (img_file, txt_file) in enumerate(zip(img_files, txt_files), 1):
            ...
            has_trigger = trigger_word.lower() in caption.lower() if trigger_word else True
            trigger_status = "✅" if has_trigger else "⚠️ "
            
            print(f"\n{i}. Image: {img_name} ↔ Caption: {txt_name}")
            print(f"   {trigger_status} Trigger: {'Present' if has_trigger else 'MISSING!'}")
            print(f"   📝 Caption ({len(caption)} chars): {caption[:100]}...")
        
        print(f"\n{'='*80}")
        print(f"✅ TRAINER WILL USE THESE .txt FILES FOR TRAINING")
        print(f"{'='*80}\n")
