        ...
        try:
            ...
            with open(jsonl_file_path, "w", encoding='utf-8') as jsonl_file:
                for i, data in enumerate(matched_data):
                    ...
                    # 🔧 FIX: Create individual .txt caption files for ai-toolkit compatibility
            ...
            print(f"   📝 Total captions (metadata.jsonl): {len(matched_data)}")
            print(f"   📄 Individual .txt files: {len(matched_data)}")
            print(f"   ✅ Dataset ready for training!")
            
            # Show sample caption files created
            print(f"\n📝 CAPTION FILES VERIFICATION:")
            for i in range(min(3, len(matched_data))):
                img_name = f"{i+1:04d}{os.path.splitext(matched_data[i]['image_name'])[1]}"
                txt_name = f"{i+1:04d}.txt"
                txt_path = os.path.join(destination_folder, txt_name)
                if os.path.exists(txt_path):
                    with open(txt_path, 'r', encoding='utf-8') as f:
                        caption_preview = f.read()[:80]
                    print(f"   ✅ {img_name} → {txt_name}")
                    print(f"      Caption: {caption_preview}...")
                else:
                    print(f"   ❌ {img_name} → {txt_name} MISSING!")
