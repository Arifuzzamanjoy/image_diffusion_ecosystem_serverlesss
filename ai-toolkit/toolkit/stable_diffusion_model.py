...
class StableDiffusion:
    ...
    def load_model(self):
        ...
        if self.model_config.is_xl or self.model_config.is_ssd or self.model_config.is_vega:
            ...
            # Use custom T5 path if specified, otherwise use default
            if self.model_config.text_encoder_2_path is not None:
                tokenizer_2 = T5TokenizerFast.from_pretrained(self.model_config.text_encoder_2_path, legacy=True, torch_dtype=dtype)
                text_encoder_2 = T5EncoderModel.from_pretrained(self.model_config.text_encoder_2_path,
                                                                torch_dtype=dtype)
            else:
            ...
            # Use custom CLIP path if specified, otherwise use default
            if self.model_config.text_encoder_path is not None:
                text_encoder = CLIPTextModel.from_pretrained(self.model_config.text_encoder_path, torch_dtype=dtype)
                tokenizer = CLIPTokenizer.from_pretrained(self.model_config.text_encoder_path, torch_dtype=dtype)
            else:
